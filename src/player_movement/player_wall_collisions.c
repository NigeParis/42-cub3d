...
/*   Updated: 2024/11/25 15:39:17 by rchourak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float get_player_radian, int player_moving)
{
	...
	next_y = fabs((cub_data->map_data->player_data.speed
				* sin(get_player_radian)));
	if (player_moving == 1
		&& treat_wall_collision_quadrant_one_west(cub_data, next_y, next_x))
		return (1);
	if (player_moving == 2
		&& treat_wall_collision_quadrant_two_west(cub_data, next_y, next_x))
		return (1);
	if (player_moving == 3
		&& treat_wall_collision_quadrant_three_west(cub_data, next_y, next_x))
		return (1);
	if (player_moving == 4
		&& treat_wall_collision_quadrant_four_west(cub_data, next_y, next_x))
		return (1);
	return (0);
}

int	detect_wall_collision_east(t_cub_data *cub_data,
float get_player_radian, int player_moving)
{
	double	next_x;
	double	next_y;
	...
	next_y = fabs((cub_data->map_data->player_data.speed
				* sin(get_player_radian)));
	if (player_moving == 1
		&& treat_wall_collision_quadrant_one_east(cub_data, next_y, next_x))
		return (1);
	if (player_moving == 2
		&& treat_wall_collision_quadrant_two_east(cub_data, next_y, next_x))
		return (1);
	if (player_moving == 3
		&& treat_wall_collision_quadrant_three_east(cub_data, next_y, next_x))
		return (1);
	if (player_moving == 4
		&& treat_wall_collision_quadrant_four_east(cub_data, next_y, next_x))
		return (1);
	return (0);
}
